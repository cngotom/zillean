...
func TestRpcJsonRequestStruct(t *testing.T) {
	...
	rpcClient.Call("setName", []string{"alex"})
	body = (<-requestChan).body
	gomega.Expect(body).To(gomega.Equal(`{"jsonrpc":"2.0","method":"setName","params":["alex"],"id":0}`))

	rpcClient.Call("setPerson", []interface{}{"alex", 33, "Germany"})
	body = (<-requestChan).body
	gomega.Expect(body).To(gomega.Equal(`{"jsonrpc":"2.0","method":"setPerson","params":["alex",33,"Germany"],"id":0}`))

	rpcClient.Call("setPersonObject", []interface{}{Person{"alex", 33, "Germany"}})
	body = (<-requestChan).body
	gomega.Expect(body).To(gomega.Equal(`{"jsonrpc":"2.0","method":"setPersonObject","params":[{"name":"alex","age":33,"country":"Germany"}],"id":0}`))

	rpcClient.Call("getDate", []interface{}{})
	body = (<-requestChan).body
	gomega.Expect(body).To(gomega.Equal(`{"jsonrpc":"2.0","method":"getDate","params":[],"id":0}`))

	rpcClient.Call("setAnonymStruct", []interface{}{struct {
...
func TestRpcJsonResponseStruct(t *testing.T) {
	...
	response, _ := rpcClient.Call("test", []interface{}{}) // Call param does not matter, since response does not depend on request
	<-requestChan
